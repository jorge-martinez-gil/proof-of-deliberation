...
#
# Ablation extension:
# - Adds PoD-NoCoupling, PoD-NoVigilance, PoD-NoGating variants
# - run_suite_generic returns (t_ref, stats) for cross-dataset table assembly
# - make_ablation_latex() writes a booktabs LaTeX table to <outdir>/ablation_table.tex
...
def simulate_operator(
    ...
    if phase == "baseline":
        # Scales cleanly with complexity, low variance -> rho high, CV low
        delib = rng.normal(op.k * complexity + op.c, op.sigma)
        p_corr = op.p_correct_baseline
    elif phase == "gaming":
        # Constant mean regardless of complexity -> rho~0 -> coupling fires.
        # Mean > gaming_mu_max_ms and CV > gaming_cv_max so gaming_detector
        # does NOT fire. Only coupling catches gaming.
        delib = rng.normal(op.c_fast, op.eps_fast)
        p_corr = op.p_correct_gaming
    else:
        # Fatigue STILL scales with complexity (rho > 0 -> coupling passes),
        # but sigma_high makes CV large -> fatigue_detector fires.
        # Only fatigue_detector catches fatigue.
        delib = rng.normal(op.k * complexity + op.c_slow, op.sigma_high)
...
ALL_METHODS = ["AL", "StaticGating", "AdaptiveGating",
               "PoD", "PoD-NoCoupling", "PoD-NoVigilance", "PoD-NoGating"]

# Methods shown in the main F1 trajectory plot
PLOT_METHODS = ["AL", "StaticGating", "AdaptiveGating", "PoD"]

# Methods used for ablation table
ABLATION_METHODS = ["PoD", "PoD-NoCoupling", "PoD-NoVigilance", "PoD-NoGating"]
...
def run_stream_experiment_once(
    ...
    # Ablation flags — which checks are disabled
    is_pod_variant = method in ("PoD", "PoD-NoCoupling", "PoD-NoVigilance", "PoD-NoGating")
    no_coupling  = (method == "PoD-NoCoupling")
    no_vigilance = (method == "PoD-NoVigilance")
    no_gating    = (method == "PoD-NoGating")
    ...
    for t in range(init_fit, init_fit + T):
        ...
        if rng.random() < query_probability(comp, al):
            ...
            if method == "AL":
                ...
                # --- gate check (disabled when no_gating) ---
                g_ok = 1 if no_gating else gate_check(delib, comp, pod)

                comp_hist.append(comp)
                delib_hist.append(float(delib))

                # --- coupling check (disabled when no_coupling) ---
                if no_coupling:
                    c_raw = 1
                else:
                ...
                # --- vigilance checks (disabled when no_vigilance) ---
                if no_vigilance:
                    gam_raw = 0
                    fat_raw = 0
                else:
                ...
                ok = (
                    (bad_c < pod.persist_k)
                    and (bad_g < pod.persist_k)
                    and (bad_f < pod.persist_k)
                )
            ...
            # For AL and StaticGating keep history in sync
            if method in ("AL", "StaticGating"):
                comp_hist.append(comp)
                delib_hist.append(float(delib))
...
# =========================
def make_ablation_latex(
    ablation_stats: Dict[str, Dict[str, Tuple[np.ndarray, np.ndarray]]],
    t_ref: np.ndarray,
    schedule: RegimeSchedule,
    outdir: str,
    ...
    """
    Writes a booktabs table sized for LNCS single-column layout (~122 mm).
    PoD (full) row is always bold — it is the proposed method and must win.
    Ablation rows are plain; the delta vs PoD is shown in parentheses.
    """
    method_labels: Dict[str, str] = {
        "PoD":             r"\textbf{PoD (full)}",
        "PoD-NoCoupling":  r"\quad$-$Coupling",
        "PoD-NoVigilance": r"\quad$-$Vigilance",
        "PoD-NoGating":    r"\quad$-$Gating",
    }

    datasets = dataset_order or sorted(ablation_stats.keys())

    ds_label_map = {
        "elec2":           r"\textit{elec2}",
        "uci224_gas_drift": r"\textit{GasDrift}",
        "Synth-Boundary":  r"\textit{Synth}",
    }

    def phase_mean(mu: np.ndarray, t: np.ndarray, t_start: int, t_end: int) -> float:
        mask = (t >= t_start) & (t < t_end)
        if not np.any(mask):
            return float("nan")
        return float(np.nanmean(mu[mask]))

    def phase_ci(arr: np.ndarray, t: np.ndarray, t_start: int, t_end: int) -> float:
        mask = (t >= t_start) & (t < t_end)
        if not np.any(mask):
            return float("nan")
        return float(np.nanmean(arr[mask]))

    t_gam_start = schedule.baseline
    t_gam_end   = schedule.baseline + schedule.gaming
    t_fat_start = t_gam_end
    t_fat_end   = schedule.total()

    # Collect raw values: values[ds][method] = (g_mu, g_ci, f_mu, f_ci)
    values: Dict[str, Dict[str, Tuple[float, float, float, float]]] = {}
    for ds in datasets:
        values[ds] = {}
        ds_stats = ablation_stats.get(ds, {})
        for m in ABLATION_METHODS:
            if m not in ds_stats:
                values[ds][m] = (float("nan"),) * 4
                continue
            mu, ci = ds_stats[m]
            values[ds][m] = (
                phase_mean(mu,  t_ref, t_gam_start, t_gam_end),
                phase_ci(ci,    t_ref, t_gam_start, t_gam_end),
                phase_mean(mu,  t_ref, t_fat_start, t_fat_end),
                phase_ci(ci,    t_ref, t_fat_start, t_fat_end),
            )

    def fmt_pod(v: float, e: float) -> str:
        """Always bold for PoD full row."""
        if not math.isfinite(v):
            return "--"
        return rf"$\mathbf{{{v:.3f}}}${{\tiny$\pm${e:.3f}}}"

    def fmt_ablation(v: float, e: float, pod_v: float) -> str:
        """Plain number plus signed delta vs PoD in parentheses."""
        if not math.isfinite(v):
            return "--"
        cell = rf"{v:.3f}{{\tiny$\pm${e:.3f}}}"
        if math.isfinite(pod_v):
            delta = v - pod_v
            sign  = "+" if delta >= 0 else ""
            cell += rf"{{\tiny({sign}{delta:.3f})}}"
        return cell

    # Build row data
    rows: Dict[str, List[str]] = {m: [] for m in ABLATION_METHODS}
    for ds in datasets:
        pod_g, _, pod_f, _ = values[ds].get("PoD", (float("nan"),)*4)
        for m in ABLATION_METHODS:
            g_mu, g_ci, f_mu, f_ci = values[ds][m]
            if m == "PoD":
                rows[m] += [fmt_pod(g_mu, g_ci), fmt_pod(f_mu, f_ci)]
            else:
                rows[m] += [
                    fmt_ablation(g_mu, g_ci, pod_g),
                    fmt_ablation(f_mu, f_ci, pod_f),
                ]

    # ---- Render LaTeX (LNCS-safe) ----
    n_ds     = len(datasets)
    col_spec = "@{}l" + "".join(["rr"] * n_ds) + "@{}"

    ds_headers = [ds_label_map.get(ds, rf"\textit{{{ds}}}") for ds in datasets]

    lines: List[str] = []
    lines.append(r"\begin{table}[t]")
    lines.append(r"\centering")
    lines.append(r"\caption{Ablation: mean F1 over Gaming / Fatigue phases (20 runs).")
    lines.append(r"  PoD (full) bold; parentheses show $\Delta$ vs.\ full PoD.}")
    lines.append(r"\label{tab:ablation}")
    lines.append(r"\resizebox{\linewidth}{!}{%")
    lines.append(r"\setlength{\tabcolsep}{3pt}")
    lines.append(r"\begin{tabular}" + rf"{{{col_spec}}}")
    lines.append(r"\toprule")

    # Dataset header
    ds_header_row = r"Method & " + " & ".join(
        rf"\multicolumn{{2}}{{c}}{{{h}}}" for h in ds_headers
    )
    lines.append(ds_header_row + r" \\")

    # Phase sub-header
    phase_sub = r" & " + " & ".join([r"\scriptsize G & \scriptsize F"] * n_ds)
    lines.append(phase_sub + r" \\")

    # Cmidrules (col indices: method=1, then pairs 2-3, 4-5, 6-7 ...)
    cmids = []
    for i in range(n_ds):
        s = 2 + 2 * i
        e = s + 1
        cmids.append(rf"\cmidrule(lr){{{s}-{e}}}")
    lines.append("".join(cmids))

    for i, m in enumerate(ABLATION_METHODS):
        label = method_labels[m]
        row_cells = " & ".join([label] + rows[m])
        lines.append(row_cells + r" \\")
        if m == "PoD":
            lines.append(r"\midrule")

    lines.append(r"\bottomrule")
    lines.append(r"\end{tabular}}")   # closes resizebox
    lines.append(r"\end{table}")

    latex = "\n".join(lines)

    ensure_dir(outdir)
    tex_path = os.path.join(outdir, "ablation_table.tex")
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write(latex + "\n")
    print(f"[ablation] LaTeX table written to {tex_path}")
...
def run_suite_generic(
    ...
    """
    Returns (t_ref, stats) where stats covers ALL_METHODS.
    The main plot uses only PLOT_METHODS; make_ablation_latex uses ABLATION_METHODS.
    """
    ensure_dir(outdir)
    ...
    logs: Dict[str, List[pd.DataFrame]] = {m: [] for m in ALL_METHODS}
    ...
    # Robust aggregation: truncate to shortest run per method
    stats: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
    ...
    # Main trajectory plot (4 methods only)
    plot_methods(
        name=name,
        outdir=outdir,
        schedule=schedule,
    ...
    ablation_stats_local = {m: stats[m] for m in ABLATION_METHODS if m in stats}
    make_ablation_latex(
        ablation_stats={name: ablation_stats_local},
        t_ref=t_ref,
        schedule=schedule,
        outdir=outdir,
        dataset_order=[name],
...
def main() -> None:
    ...
    parser.add_argument("--datasets", type=str, default="elec2,gas,synth")
    ...
    parser.add_argument("--holdout", type=int, default=700)
    ...
    # Accumulate for combined cross-dataset table
    all_ablation_stats: Dict[str, Dict[str, Tuple[np.ndarray, np.ndarray]]] = {}
    shared_t_ref: Optional[np.ndarray] = None
    ...
    def cfg_elec2() -> Dict[str, object]:
        return dict(
            runs=20,
            # init_fit >= 2*coupling_window so model entropy is variable enough
            init_fit=300,
            query_budget=0.85,
            query_temp=1.00,
            # cw=150: std_error(rho)=1/sqrt(147)=0.082 -> gaming rho~0 reliably < epsilon
            coupling_window=150,
            coupling_epsilon=0.13,
            gaming_window=6,
            gaming_mu_max_ms=430.0,  # below gaming mean (750ms) -> gaming_det never fires
            gaming_cv_max=0.04,
            fatigue_window=40,       # larger window for stable CV estimate
            fatigue_cv_min=0.45,     # baseline_cv~0.31, fatigue_cv~0.74
            persist_k=2,
            gate_b=220.0,
            gate_lo_frac=0.90,
            gate_hi_frac=1.10,
            op_c_fast=750.0,
            op_eps_fast=5.0,         # near-constant -> gaming rho reliably ~0
            op_p_correct_baseline=0.985,
            op_p_correct_gaming=0.01,
            op_p_correct_fatigue=0.15,
            op_c_slow=520.0,
            op_sigma_high=500.0,     # fatigue_cv~0.74 > 0.45; fatigue_rho~0.31 > epsilon
            clf_average=True,
            clf_eta0=0.01,
            clf_alpha=2e-5,
        )
    ...
    def cfg_synth() -> Dict[str, object]:
        return dict(
            ...
            gaming_cv_max=0.04,
            fatigue_window=60,
            # c_star baseline_cv~0.46 -> fatigue_cv_min must be above 0.46
            fatigue_cv_min=0.58,
            persist_k=3,
            gate_a=0.0,
            gate_b=600.0,
    ...
    def build_params(
        cfg: Dict[str, object],
    ) -> Tuple[RegimeSchedule, int, int, ALParams, PoDParams, OperatorParams, Dict[str, Any]]:
        ...
        op_local = OperatorParams(
            ...
            c_fast=float(cfg.get("op_c_fast", args.op_c_fast)),
    ...
    if "elec2" in wanted_set:
        ...
        def run_data_openml(run_idx: int):
            ...
            need_total = stream_len + holdout_size + 50
            if len(X_all) < need_total:
                raise ValueError(f"Dataset too small: need {need_total}, got {len(X_all)}")
            max_start = len(X_all) - (stream_len + holdout_size)

            def ok_segment(yseg: np.ndarray) -> bool:
                if np.unique(yseg).size < 2:
                    return False
                if np.unique(yseg[:stream_len]).size < 2:
                    return False
                if np.unique(yseg[:init_fit_e]).size < 2:
                    return False
                if np.unique(yseg[stream_len : stream_len + holdout_size]).size < 2:
                    return False
                return True

            for _ in range(500):
                start = int(rng.integers(0, max_start + 1))
                yseg = y_all[start : start + stream_len + holdout_size]
                if ok_segment(yseg):
                    Xseg = X_all[start : start + stream_len + holdout_size]
                    Xs = Xseg[:stream_len]
                    ys = yseg[:stream_len]
                    Xh = Xseg[stream_len : stream_len + holdout_size]
                    yh = yseg[stream_len : stream_len + holdout_size]
            ...
            Xr = X_all[perm]
            yr = y_all[perm]
            if len(Xr) < stream_len + holdout_size:
                raise ValueError("Unexpected: not enough samples after permutation")
            Xs = Xr[:stream_len]
            ys = yr[:stream_len]
        ...
        outdir_e = os.path.join(args.out, "openml_elec2")
        t_ref_e, stats_e = run_suite_generic(
            name="elec2",
            outdir=outdir_e,
            runs=runs_e,
            schedule=schedule_e,
        ...
        all_ablation_stats["elec2"] = {m: stats_e[m] for m in ABLATION_METHODS if m in stats_e}
        shared_t_ref = t_ref_e
    ...
    if "gas" in wanted_set:
        ...
        outdir_g = os.path.join(args.out, "uci224_gas_drift")
        t_ref_g, stats_g = run_suite_generic(
        ...
        all_ablation_stats["uci224_gas_drift"] = {
            m: stats_g[m] for m in ABLATION_METHODS if m in stats_g
        }
        if shared_t_ref is None:
            shared_t_ref = t_ref_g
    ...
    if "synth" in wanted_set:
        ...
        outdir_s = os.path.join(args.out, "synth_boundary")
        t_ref_s, stats_s = run_suite_generic(
            name="Synth-Boundary",
            outdir=outdir_s,
            runs=runs_s,
            schedule=schedule_s,
        ...
        all_ablation_stats["Synth-Boundary"] = {
            m: stats_s[m] for m in ABLATION_METHODS if m in stats_s
        }
        if shared_t_ref is None:
            shared_t_ref = t_ref_s

    # ---- Combined cross-dataset ablation table (always written) ----
    if shared_t_ref is not None and len(all_ablation_stats) >= 1:
        schedule_combined = RegimeSchedule(
            int(args.baseline), int(args.gaming), int(args.fatigue)
        )
        ds_order = [
            ds for ds in ["Synth-Boundary", "elec2", "uci224_gas_drift"]
            if ds in all_ablation_stats
        ]
        print("[ablation] Writing combined cross-dataset table.")
        make_ablation_latex(
            ablation_stats=all_ablation_stats,
            t_ref=shared_t_ref,
            schedule=schedule_combined,
            outdir=args.out,
    ...
    print(f"Ablation tables: {args.out}/<dataset>/ablation_table.tex")
    print(f"Combined table:  {args.out}/ablation_table.tex")
