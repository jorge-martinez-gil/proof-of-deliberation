...
#
# Change for elec2 variance:
# - Keep OpenML order (no dataset-wide shuffling in load_openml_any)
# - For each run: sample a random contiguous segment for the stream
# - Holdout is the immediate "future" block after the stream segment (no random holdout mixing)
...
def load_openml_any(spec: OpenMLSpec, seed: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Variance change: keep original order (no shuffling here).
    # The seed remains in signature for compatibility.
    _ = seed
    return X, y
    ...
    if "elec2" in wanted_set:
        ...
        def run_data_openml(run_idx: int):
            ...
            def ok_segment(yseg: np.ndarray) -> bool:
                # Must have >=2 classes overall
                if np.unique(yseg).size < 2:
                    return False
                # Must have >=2 classes in stream block (otherwise training/eval is degenerate)
                if np.unique(yseg[:stream_len]).size < 2:
                    return False
                # Must have >=2 classes in the initial fit (avoid pathological warmup)
                if np.unique(yseg[:init_fit_e]).size < 2:
                    return False
                # Must have >=2 classes in holdout block
                if np.unique(yseg[stream_len : stream_len + holdout_size]).size < 2:
                    return False
                return True

            # Try to find a valid contiguous segment
            for _ in range(500):
            ...
            # Fallback: keep the run alive even if order has huge single-class stretches
            # This reduces variance vs contiguous, but avoids crashing.
