...
#
# This version adds:
# - Ablation evidence export: per-run metrics + aggregated summary + deltas vs reference
# - LaTeX tables written to disk (ready to paste into paper)
# - Fix: elec2 now actually runs run_suite_generic (previously it only defined run_data_openml)
#
# Outputs per dataset directory:
# - ablation_metrics_raw.csv
# - ablation_metrics_with_diagnostics.csv
# - ablation_summary.csv
# - ablation_deltas_vs_PoD.csv
# - ablation_summary.tex
# - ablation_deltas_vs_PoD.tex
#
# Notes:
# - LaTeX uses booktabs; add \usepackage{booktabs} in your preamble.
# - Tables are designed to be pasted as-is.
#
...
def auc_trapz(t: np.ndarray, y: np.ndarray) -> float:
    t = np.asarray(t, dtype=float)
    y = np.asarray(y, dtype=float)
    if len(t) < 2:
        return float(y[-1]) if len(y) == 1 else 0.0
    return float(np.trapz(y, t))


def mean_in_window(t: np.ndarray, y: np.ndarray, lo: int, hi: int) -> float:
    t = np.asarray(t, dtype=int)
    y = np.asarray(y, dtype=float)
    mask = (t >= int(lo)) & (t < int(hi))
    if not np.any(mask):
        return float("nan")
...
def write_latex_tables(outdir: str, summary_df: pd.DataFrame, deltas_df: pd.DataFrame, ref: str) -> None:
    # Summary table: method vs key metrics
    # Keep columns compact and paper-friendly
    cols = [
        "method",
        "f1_final",
        "f1_mean",
    ...
    sd = summary_df.copy()
    sd = sd.sort_values("method")

    # Build pretty columns as strings
    out = pd.DataFrame()
    out["Method"] = sd["method"].astype(str)
    out["F1 final"] = [fmt_pm(m, c, 3) for m, c in zip(sd["f1_final_mean"], sd["f1_final_ci95"])]
    out["F1 mean"] = [fmt_pm(m, c, 3) for m, c in zip(sd["f1_mean_mean"], sd["f1_mean_ci95"])]
    out["AUC"] = [fmt_pm(m, c, 3) for m, c in zip(sd["auc_mean"], sd["auc_ci95"])]
    out["F1 gaming"] = [f"{x:.3f}" if np.isfinite(x) else "" for x in sd["f1_gaming_mean"].to_numpy()]
    out["F1 fatigue"] = [f"{x:.3f}" if np.isfinite(x) else "" for x in sd["f1_fatigue_mean"].to_numpy()]
    out["Query rate"] = [f"{x:.3f}" if np.isfinite(x) else "" for x in sd["query_rate_mean"].to_numpy()]
    out["Accept | query"] = [f"{x:.3f}" if np.isfinite(x) else "" for x in sd["accept_rate_given_query_mean"].to_numpy()]

    tex_summary = out.to_latex(
        index=False,
        escape=False,
        booktabs=True,
        column_format="lccccccc",
    )
    with open(os.path.join(outdir, "ablation_summary.tex"), "w", encoding="utf-8") as f:
        f.write(tex_summary)

    # Deltas table: ref vs other for selected metrics
    if deltas_df is not None and len(deltas_df) > 0:
        dd = deltas_df.copy()
        dd = dd.sort_values(["metric", "other"])

        metric_order = ["f1_final", "f1_mean", "auc", "f1_gaming", "f1_fatigue"]
        metric_name = {
            "f1_final": "F1 final",
            "f1_mean": "F1 mean",
            "auc": "AUC",
            "f1_gaming": "F1 gaming",
            "f1_fatigue": "F1 fatigue",
        }
        dd["metric_rank"] = dd["metric"].map({m: i for i, m in enumerate(metric_order)}).fillna(999).astype(int)
        dd = dd.sort_values(["metric_rank", "other"])

        out2 = pd.DataFrame()
        out2["Metric"] = [metric_name.get(x, x) for x in dd["metric"].astype(str)]
        out2["Other"] = dd["other"].astype(str)
        out2[f"{ref} - Other"] = [fmt_pm(m, c, 3) for m, c in zip(dd["delta_mean"], dd["delta_ci95"])]
        out2["Paired d"] = [f"{x:.3f}" if np.isfinite(x) else "" for x in dd["paired_d"].to_numpy()]

        tex_deltas = out2.to_latex(
            index=False,
            escape=False,
            booktabs=True,
            column_format="l l c c",
        )
        with open(os.path.join(outdir, f"ablation_deltas_vs_{ref}.tex"), "w", encoding="utf-8") as f:
            f.write(tex_deltas)
...
def run_suite_generic(
    ...
    diag_df.to_csv(os.path.join(outdir, "diagnostics.csv"), index=False)

    # Robust aggregation for plotting: truncate all runs per method to the shortest length
    stats: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
    ...
    # =========================
    # Ablation evidence export
    # =========================
    rows = []
    for m in methods:
        for r_idx, df in enumerate(logs[m]):
            if len(df) == 0:
                continue
            t = df["t"].to_numpy(dtype=int)
            f1v = df["f1"].to_numpy(dtype=float)

            b0, b1 = 0, schedule.baseline
            g0, g1 = schedule.baseline, schedule.baseline + schedule.gaming
            f0, f1w = schedule.baseline + schedule.gaming, schedule.total()
            ...
            rows.append(
                dict(
                    ...
                    method=m,
                    n_eval=int(len(f1v)),
                    f1_final=float(f1v[-1]),
                    f1_mean=float(np.mean(f1v)),
                    f1_p05=float(np.quantile(f1v, 0.05)) if len(f1v) >= 2 else float(f1v[-1]),
                    auc=float(auc_trapz(t, f1v)),
                    f1_baseline=float(mean_in_window(t, f1v, b0, b1)),
                    f1_gaming=float(mean_in_window(t, f1v, g0, g1)),
                    f1_fatigue=float(mean_in_window(t, f1v, f0, f1w)),
                )
            )

    curves_metrics = pd.DataFrame(rows)
    curves_metrics.to_csv(os.path.join(outdir, "ablation_metrics_raw.csv"), index=False)

    merged = curves_metrics.merge(diag_df, on=["dataset", "run", "method"], how="left")
    merged.to_csv(os.path.join(outdir, "ablation_metrics_with_diagnostics.csv"), index=False)

    summ_rows = []
    for m in methods:
        mm = merged[merged["method"] == m]
        if len(mm) == 0:
            continue
        summ_rows.append(
            dict(
                dataset=name,
                method=m,
                runs=int(mm["run"].nunique()),
                f1_final_mean=float(mm["f1_final"].mean()),
                f1_final_ci95=float(ci95(mm["f1_final"].to_numpy())),
                f1_mean_mean=float(mm["f1_mean"].mean()),
                f1_mean_ci95=float(ci95(mm["f1_mean"].to_numpy())),
                auc_mean=float(mm["auc"].mean()),
                auc_ci95=float(ci95(mm["auc"].to_numpy())),
                f1_baseline_mean=float(mm["f1_baseline"].mean()),
                f1_gaming_mean=float(mm["f1_gaming"].mean()),
                f1_fatigue_mean=float(mm["f1_fatigue"].mean()),
                query_rate_mean=float(mm["query_rate"].mean()) if "query_rate" in mm else float("nan"),
                accept_rate_given_query_mean=float(mm["accept_rate_given_query"].mean())
                if "accept_rate_given_query" in mm
                else float("nan"),
            )
        )

    summary = pd.DataFrame(summ_rows)
    summary.to_csv(os.path.join(outdir, "ablation_summary.csv"), index=False)
    ...
    for m in methods:
        if m == ref:
            continue
        a = merged[merged["method"] == ref].sort_values("run")
        b = merged[merged["method"] == m].sort_values("run")
        common = sorted(set(a["run"]).intersection(set(b["run"])))
        if not common:
            continue
        a = a[a["run"].isin(common)].sort_values("run")
        b = b[b["run"].isin(common)].sort_values("run")

        for metric in ["f1_final", "f1_mean", "auc", "f1_gaming", "f1_fatigue"]:
            ...
            deltas.append(
                dict(
                    ...
                    delta_ci95=float(ci95(diff)),
                    paired_d=float(paired_cohens_d(da, db)),
                )
            )

    deltas_df = pd.DataFrame(deltas)
    deltas_df.to_csv(os.path.join(outdir, f"ablation_deltas_vs_{ref}.csv"), index=False)

    write_latex_tables(outdir=outdir, summary_df=summary, deltas_df=deltas_df, ref=ref)
...
def main() -> None:
    ...
    parser.add_argument("--datasets", type=str, default="elec2")
    ...
    if "elec2" in wanted_set:
        ...
        outdir = os.path.join(args.out, "openml_elec2")
        run_suite_generic(
            name="elec2",
            outdir=outdir,
            runs=runs_e,
    ...
    print("Done.")
    print(f"Outputs under: {args.out}/<dataset>/")
